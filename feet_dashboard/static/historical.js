      ...
      for (let i = 0; i < seg_ids.length; i++) {
        ...
        for (let j = 0; j < markers.length; j++) {
          ...
          if (marker.type === "start") {
            const is_first = (marker.order === 1);
            const label = is_first ? "Start" : marker.order;
            
            const start_icon = L.divIcon({
                html: `<div style="background-color: green; color: white; border-radius: 50%; width: 30px; height: 30px; display: flex; justify-content: center; align-items: center; font-size: 12px; font-weight: bold; border: 2px solid white;">${label}</div>`,
                className: "",
                iconSize: [22, 22]
            });
            ...
            const is_last = (i === seg_ids.length - 1);
            const label = is_last ? "End" : "";
            
            if (is_last) {
                const end_icon = L.divIcon({
                    html: `<div style="background-color: red; color: white; border-radius: 50%; width: 30px; height: 30px; display: flex; justify-content: center; align-items: center; font-size: 12px; font-weight: bold; border: 2px solid white;">${label}</div>`,
                    className: "",
                    iconSize: [22, 22]
                });
                const m = L.marker([marker.lat, marker.lon], { icon: end_icon }).addTo(map);
                window.segmentMarkers.push(m);
            } else {
                const m = L.circleMarker([marker.lat, marker.lon], { color: "orange", radius: 6 }).addTo(map);
                window.segmentMarkers.push(m);
            }
        } else if (marker.type === "failure") {
          const fail_icon = L.divIcon({
              html: `<div style="width: 6px; height: 6px; background-color: red; border-radius: 50%; opacity: 0.8;"></div>`,
              className: "",
              iconSize: [6, 6]
          });
          const m = L.marker([marker.lat, marker.lon], { icon: fail_icon }).addTo(map);
          window.segmentMarkers.push(m);
      }
        }
      }
